...
export default function App() {
  ...
  async function analyze() {
    if (!imageUri) return;
    if (!OPENAI_API_KEY || OPENAI_API_KEY.startsWith("sk-") === false) {
      return Alert.alert("Missing API Key", "Add your OPENAI_API_KEY to the .env file and restart with `expo start -c`.");
    }
    ...
    try {
      ...
      // Read as base64 and wrap in a data URL
      const b64 = await FileSystem.readAsStringAsync(processed.uri, { encoding: "base64" });
      const dataUrl = `data:image/jpeg;base64,${b64}`;

      // Call OpenAI Vision (Chat Completions with image input)
      const messages = [
...
No markdown, no comments—just JSON.
            `.trim() },
            { type: "input_image", image_data: dataUrl }
          ]
        }
      ];
